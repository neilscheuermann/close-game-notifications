...
// during game hours.
cron.schedule(`${outerCronMin} ${outerCronHour} * * *`, () => {
  ...
  dailyCronJobSchedule(todaysGames).then(startEnd => {
    ...
    console.log(
      `*** Inner cron job will check scores every 15 seconds today from ${start}:00 through ${end}:59. (ran at ${new Date()}***`
    )
    ...
    cron.schedule(`*/15 * ${start}-${end} * * *`, () => {
      ...
      // games.
      todaysGames()
        .then(data => createMessagesForNailBiterGames(data))
        .then(messagesArr => {
          ...
          // Loops through each object of the nail-biter array.
            ...
            messagesArr.forEach(message => {
              ...
              if (!messageHasBeenSent[message.gameId]) {
                ...
                console.log(
                  `The following message has been sent to subscribers: ${
                    message.message
                ...
                console.log(
                  `Message for the ${
                    message.teamsPlaying
                  } game has already been sent.`
                )
              }
            })
        })
